/*	This program modifys Project 11 to add a new search option to the menu that:
        1) Display the first and last name (sorted by last name).
        2) Get the last name to search for from the user.
        3) Display student information, but also include first and last names.
        
	Filename: final02Marrujo.c
...
// Inputs
void readData(char firstName[][20], char lastName[][20], int studentID[], int studentScores[][NUM_SCORES]);
int getData(char firstName[][20], char lastName[][20], int studentID[], int studentScores[][NUM_SCORES], 
            int numCurrentStudents);
// Reports 
void reportOutput( char lastName[][20], char firstName[][20], int studentID[], int studentScores[][NUM_SCORES], 
                  int studentMinScore[], int studentMaxScore[], float studentAvg[], int numCurrentStudents, 
                  char studentLetterGrade[]);
void displayTestStatistics(int testMax[], int testMin[], float testAvg[]);
// Calcs
void calcAve(int studentScores[][NUM_SCORES], float studentAvg[MAX_STUDENTS], int numCurrentStudents);
void calcMinScore(int studentScores[][NUM_SCORES], int studentMaxScore[], int numCurrentStudents);								
void calcMaxScore(int studentScores[][NUM_SCORES], int studentMinScore[], int numCurrentStudents);
void findGrade(float studentAvg[], char studentLetterGrade[], int numCurrentStudents);								
void calcMaxScore(int studentScores[][NUM_SCORES], int studentMinScore[], int numCurrentStudents);								
void calcTestStatistics(int studentScores[][NUM_SCORES], int testMax[], int testMin[], float testAvg[], int numCurrentStudents);
// Validation
bool checkChar(char lastName);
bool checkIntRange(int integer);
// Search
void searchForStudentID(char lastName[][20], char firstName[][20], int studentID[], int studentScores[][NUM_SCORES], int studentMinScore[], int studentMaxScore[], 
                      float studentAvg[], int numCurrentStudents, char studentLetterGrade[]);
void searchOptionsID(char lastName[][20], char firstName[][20], int studentID[], int studentScores[ ][NUM_SCORES], int studentMinScore[ ], int studentMaxScore[ ], 
					float studentAvg[ ], int numCurrentStudents, char studentLetterGrade[ ]);
void searchForStudentLastName(char lastName[][20], char firstName[][20], int studentID[], int studentScores[][NUM_SCORES], int studentMinScore[], int studentMaxScore[], 
                      float studentAvg[], int numCurrentStudents, char studentLetterGrade[]);;
void searchOptionsLastName(char lastName[][20], char firstName[][20], int studentID[], int studentScores[ ][NUM_SCORES], int studentMinScore[ ], int studentMaxScore[ ], 
					float studentAvg[ ], int numCurrentStudents, char studentLetterGrade[ ]);
...
int main(void)
{
    ...
    char firstName[MAX_STUDENTS][20];
    char lastName[MAX_STUDENTS][20];
    int studentID[MAX_STUDENTS];
    int studentScores[MAX_STUDENTS][NUM_SCORES];
    int studentMaxScore[MAX_STUDENTS];
    int studentMinScore[MAX_STUDENTS];
    float studentAvg[MAX_STUDENTS];
    char studentLetterGrade[MAX_STUDENTS];

    int testMax[NUM_SCORES];
    int testMin[NUM_SCORES];
    float testAvg[NUM_SCORES];
    
    // Statements ================

    printf("Welcome to The Student Report.\n");
    
    readData(firstName, lastName, studentID, studentScores);
    calcAve(studentScores, studentAvg, numCurrentStudents);
	calcMinScore( studentScores, studentMinScore, numCurrentStudents);
	calcMaxScore( studentScores, studentMaxScore, numCurrentStudents);
    ...
    {   
        ...
        printf("4.   Search for a student (By lastName)\n");
        printf("5.   Quit\n\n");
        ...
        if (isdigit(userInput) && userInput == '1')
        {
			...
			reportOutput( lastName, firstName, studentID, studentScores, studentMinScore, studentMaxScore, studentAvg, 
									numCurrentStudents, studentLetterGrade);
			// calculate and display test statistics
			calcTestStatistics(studentScores, testMax, testMin, testAvg, numCurrentStudents );
			displayTestStatistics(testMax, testMin, testAvg );
		...
		else if (isdigit(userInput) && userInput == '2')
		{
			...
			{
				numCurrentStudents = getData( lastName, firstName, studentID, studentScores, numCurrentStudents);
			}// if there is room
			else
			{
		...
		else if (isdigit(userInput) && userInput == '3')
		{
		    ...
		    searchOptionsID(lastName, firstName, studentID, studentScores, studentMinScore, studentMaxScore, studentAvg, numCurrentStudents, studentLetterGrade);
			searchForStudentID( lastName, firstName, studentID, studentScores, studentMinScore, studentMaxScore, studentAvg, numCurrentStudents, studentLetterGrade);
		}// else if
        else if (isdigit(userInput) && userInput == '4')
		{
			printf("\nAvailable Student Information\nLast	   First");
		    searchOptionsLastName(lastName, firstName, studentID, studentScores, studentMinScore, studentMaxScore, studentAvg, numCurrentStudents, studentLetterGrade);
			searchForStudentLastName( lastName, firstName, studentID, studentScores, studentMinScore, studentMaxScore, studentAvg, numCurrentStudents, studentLetterGrade);
		}// else if
		else if (isdigit(userInput) && userInput == '5')
		{
			// end program
		...
		{
			printf("Incorrect input. Please enter 1, 2, 3, 4 or 5.\n");
			FLUSH;
		}// else
...
/* =================================================================================
	...
	Update: o Initials were changed to full names 04/22/23
	        o Made it so Names are now capitalized 04/24/23
====================================================================================*/
void readData(char firstName[][20], char lastName[][20], int studentID[], int studentScores[][NUM_SCORES])
{	
    // Local Definitions ==============
	...
	{
		// read a student
		fscanf(spData," %s %s %d %d %d %d %d", firstName[row], lastName[row], &studentID[row], 
			&studentScores[row][0], &studentScores[row][1], &studentScores[row][2], &studentScores[row][3]);
		for(int i=0;i<20;i++)
	    {
		    firstName[row][i] = toupper(firstName[row][i]);
	    }
		for(int i=0;i<20;i++)
	    {
			lastName[row][i]  = toupper(lastName[row][i]);
	    }
	}// for
 ...
 /* =================================================================================
	...
	Updates: o Initials were changed to full names 04/22/23
	         o Now properly reads input names 04/24/23
====================================================================================*/
int getData( char lastName[][20], char firstName[][20], int studentID[ ], int studentScores[ ][NUM_SCORES], 
							int numCurrentStudents)
{
	// Local Definitions ==============
	int scanCount = 0;
	...
	while (isValid == false)
	{
		...
		scanf("%s", lastName[newStudentIndex]);
		if(lastName[newStudentIndex][0] == ']')
		{
			lastName[newStudentIndex][0] = ' ';
			printf("Quitting Data Entry...\n");
			return numCurrentStudents;
		}// if quit data entry
		if (checkChar(lastName[newStudentIndex][0]))
		{
		    for(int i=0;i<20;i++)
		    {
    			lastName[newStudentIndex][i]  = toupper(lastName[newStudentIndex][i]);
    			isValid = true;
		    }
		}// if
	...
	while (isValid == false)
	{
		...
		scanf("%s", firstName[newStudentIndex]);
		if(firstName[newStudentIndex][0] == ']')
		{
			firstName[newStudentIndex][0] = ' ';
			printf("Quitting Data Entry...\n");
			return numCurrentStudents;
		}// if quit data entry
		if (checkChar(firstName[newStudentIndex][0]))
		{
		    for(int i=0;i<20;i++)
		    {
			    firstName[newStudentIndex][i] = toupper(firstName[newStudentIndex][i]);
			    isValid = true;
		    }
		}// if
					...
					char studentLetterGrade[])
{
	...
	for (int i = 0; i < numCurrentStudents-1; i++)
	{
		for (int j = i+1; j < numCurrentStudents; j++)
		{
			if (strcmp(lastName[i], lastName[j]) > 0)
			{
				...
				{
					int scoreTemp = studentScores[i][k];
					studentScores[i][k] = studentScores[j][k];
					studentScores[j][k] = scoreTemp;
				}
				
				// Swap min score
				int minScoreTemp = studentMinScore[i];
				studentMinScore[i] = studentMinScore[j];
				studentMinScore[j] = minScoreTemp;
				
				// Swap max score
				int maxScoreTemp = studentMaxScore[i];
				studentMaxScore[i] = studentMaxScore[j];
				studentMaxScore[j] = maxScoreTemp;
				
				// Swap avg
				float avgTemp = studentAvg[i];
				studentAvg[i] = studentAvg[j];
				studentAvg[j] = avgTemp;
				
				// Swap letter grade
				char gradeTemp = studentLetterGrade[i];
				studentLetterGrade[i] = studentLetterGrade[j];
				studentLetterGrade[j] = gradeTemp;
			}
		}
	}
	...
	for (int row = 0; row < numCurrentStudents; row++)
	{
		printf("\n%-9s %-9s %-8d %-8d %-8d %-8d %-8d %-8.1d %-8.1d %-8.1f %-8c", lastName[row], firstName[row], studentID[row],
...
bool checkIntRange(int intValue)
{
 ...
 /* =================================================================================
	...
	Date: April 2nd, 2023
====================================================================================*/
void calcMinScore( int studentScores[ ][NUM_SCORES], int studentMinScore [ ],int numCurrentStudents)
{
	// Local Definitions ==============
	int minScore = 101;
 ...
 /* =================================================================================
	...
	Date: April 2nd, 2023
====================================================================================*/
void calcMaxScore( int studentScores[ ][NUM_SCORES], int studentMaxScore [ ],int numCurrentStudents)
{
	// Local Definitions ==============
	int maxScore = -99;
					...
					float studentAvg[ ], int numCurrentStudents, char studentLetterGrade[ ])
{
	...
	if (quit == false)
	{
		if (isValid)
		{
			...
			printf("\n%-8d %-9s %-9s %-8d %-8d %-8d %-8d %-8.1d %-8.1d %-8.1f %-8c", studentID[row], lastName[row], firstName[row],
					studentScores[row][0], studentScores[row][1], studentScores[row][2], studentScores[row][3], studentMaxScore[row],
					studentMinScore[row], studentAvg[row], studentLetterGrade[row]);
		}// if
		else
					...
					float studentAvg[ ], int numCurrentStudents, char studentLetterGrade[ ])
{
	...
	for (int row = 0; row < numCurrentStudents; row++)
	{
		printf("\n%-8d %-9s %-9s ", studentID[row], lastName[row], firstName[row]);
	}
	
}//searchOptionsID

/* =================================================================================
Function Name: searchForStudentLastName
Function Description: gets student last name from user (with error-checking), searches for student and displays
an appropriate message if the student is not found, or the student last name, test scores, min, max, average and
letter grade for the found student.
Inputs: pass-by-reference pointers to student's data
Outputs: None - uses pass-by-reference
Date: April 30th, 2023
                            ...
                            float studentAvg[], int numCurrentStudents, char studentLetterGrade[])
{
    ...
    {
        FLUSH;
        printf("\n\nEnter the student's last name to search for (use ] to quit): ");
        tempInput = getc(stdin);
        if (tempInput == ']')
        {
            FLUSH;
        ...
        else
        {
            for(int i=0;i<20;i++)
    	    {
    			searchLastName[i]  = toupper(searchLastName[i]);
    	    }
            isValid = true;
        }// else
    }// while getting last name
    
    // search for last name in lastName ======================
    isValid = false;
    while (isValid == false && currCount < numCurrentStudents && quit == false)
    {
        if (strcmp(searchLastName, lastName[row]) == 0)
        {
            isValid = true;
        }// if
        else
    ...
    if (quit == false)
    {
        ...
        {
            printf("\nSubmitted Student last name not found...\n");
    		}// else
    	}// if
	return;
}// searchForStudentLastName

 /* =================================================================================
	Function Name: searchOptionsLastName
	Function Description: display stundents available for search
	Inputs: pass-by-reference pointers to student's data
	Outputs: None - uses pass-by-reference
	Date: April 30th, 2023
					...
					float studentAvg[ ], int numCurrentStudents, char studentLetterGrade[ ])
{
	...
	for (int row = 0; row < numCurrentStudents; row++)
	{
		printf("\n%-9s %-9s ", lastName[row], firstName[row]);
	}
	
}//searchOptionsLastName
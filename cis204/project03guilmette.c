/*	A class has four assessments worth equal value. The class has 3 students. 
		...
		formatted output that is clear to read.
	File Name: project03guilmette.c
...
int main(void)
{
	...
	scanf(" %c,%c,%d,%d,%d,%d,%d,%f,%f", &lastName1, &firstName1, &id1, 
		&test1_1, &test1_2, &test1_3, &test1_4, &firstHigh, &firstLow);
	// Prompt and get the second student's data
	printf("Please enter the second student's data all on one line in the sequence\n");
	scanf(" %c,%c,%d,%d,%d,%d,%d,%f,%f", &lastName2, &firstName2, &id2, 
		&test2_1, &test2_2, &test2_3, &test2_4, &secondHigh, &secondLow);
	// Prompt and get the third student's data
	printf("Please enter the second student's data all on one line in the sequence\n");
	scanf(" %c,%c,%d,%d,%d,%d,%d,%f,%f", &lastName3, &firstName3, &id3, 
		&test3_1, &test3_2, &test3_3, &test3_4, &thirdHigh, &thirdLow);

	// Calcluate the first student's average
	firstAve = (test1_1 + test1_2 + test1_3 + test1_4)/numTests;
	// Calcluate the second student's average
	secondAve = (test2_1 + test2_2 + test2_3 + test2_4)/numTests;
	// Calcluate the second student's average
	thirdAve = (test3_1 + test3_2 + test3_3 + test3_4)/numTests;

	// Extra Stuff - round to one decimal place =================
	firstAve = roundf(firstAve*10)/10.0;
	secondAve = roundf(secondAve*10)/10.0;
	thirdAve = roundf(thirdAve*10)/10.0;
	// End Extra Stuff ==========================================
